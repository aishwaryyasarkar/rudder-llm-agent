...
def direction_to_sign(direction_val):
    ...
    if s in ["increase", "increased", "+", "+1", "up"]:
        return 1
    elif s in ["decrease", "decreased", "-", "-1", "down", "lower", "less"]:
        return -1
    elif s in ["remain", "unchanged", "stable", "same", "0", "stagnant", "no change"]:
        return 0
    return None

###############################################################################
# 2b) infer expectations for NON-LLM agents (NEW)
###############################################################################
def infer_expectation_from_decision(decision_str: str):
    d = decision_str.strip().lower().rstrip(".")
    if d == "yes, evict":
        return 1, None   # expect ΔHR > 0, no comm scoring
    if d == "no, do not evict":
        return 0, None   # expect ΔHR >= 0 (we treat as 0; accept actual 0 or +1)
    return None, None

###############################################################################
# 3) parse_decision (supports both LLM JSON and NON-LLM plain text) (UPDATED)
###############################################################################
...
def parse_decision(text: str, agent_type: str):
    ...
    # Try JSON first (LLM path)
    start_idx = between.find('{')
    end_idx = between.rfind('}')
    if agent_type == "llm" and start_idx != -1 and end_idx != -1 and end_idx > start_idx:
    ...
    # Non-JSON / NON-LLM: treat as raw decision text
    raw = between.strip()
    if not raw:
        return None
    if agent_type == "nonllm":
        ph, pc = infer_expectation_from_decision(raw)
        return {"decision": raw.lower(), "pred_hr_dir": ph, "pred_comm_dir": pc}
    # LLM without JSON: decision but no explicit expectations
    return {"decision": raw.lower(), "pred_hr_dir": None, "pred_comm_dir": None}

###############################################################################
# 4) parse_latest_metrics (UPDATED to support your keys)
###############################################################################
def parse_latest_metrics(block):
    hr_val = None
    comm_val = None

    # First: your top-of-chunk dict with Pre_Avg_* keys
    ...
    if m_hr:
        try:
            ...
            pass
    m_comm = re.search(r"[\'\"]Pre_Avg_T_rpc[\'\"]\s*:\s*([-+]?\d+(?:\.\d+)?)", block)
    if m_comm:
        try:
    ...
    # Fallback: your older "Latest Metrics: {'hitrate': ..., 'comm_volume': ...}"
    if hr_val is None or comm_val is None:
...
###############################################################################
# 5) extract_minibatch_id (UPDATED to your canonical format)
###############################################################################
def extract_minibatch_id(chunk):
    ...
    if m:
        try:
            ...
            return None
    # Optional fallbacks (keep or remove as you prefer)
...
###############################################################################
# 6) parse_one_record (UPDATED to accept agent_type)
###############################################################################
def parse_one_record(record, agent_type):
    rt_match = re.search(r"(?i)response\s*time[:\s]+([\d\.]+)s", record)
    if not rt_match:
        return None
    response_time = float(rt_match.group(1))

    decision_data = parse_decision(record, agent_type)
...
###############################################################################
# 8) process_file (UPDATED: non-LLM relaxed rule on "no, do not evict")
###############################################################################
def process_file(filename, rank2finalhr, agent_type):
    ...
    for i in range(len(parsed) - 1):
        ...
        # --- Comm correctness (only if a prediction exists) ---
    ...
    # Average minibatch interval in this file
    minibatch_ids = []
    for c in chunks:
...
###############################################################################
# 9) evaluate_and_return_stats (UNCHANGED except signature passthrough)
###############################################################################
def evaluate_and_return_stats(subdir, agent_type):
    ...
    for file in sorted(files):
        summary = process_file(file, rank2finalhr, agent_type)
    ...
    # Per-run "File-Level CI" via chi-square on per-file HR accuracies
    ...
    overall_stats = {
        ...
        "overall_hr_file_ci": overall_hr_file_ci,
...
###############################################################################
# 10) main (UPDATED: adds --agent-type and passes it through)
###############################################################################
def main():
    parser = argparse.ArgumentParser(
        description="Stats + File-Level CI per-run, and GLOBAL Per-Record binomial CI across all runs. Supports LLM and non-LLM agents."
    )
    ...
    parser.add_argument("--agent-type", choices=["llm", "nonllm"], default="llm",
                        help="Agent type: 'llm' (JSON with expected_impact) or 'nonllm' (plain decisions).")
            ...
            for root, dirs, files in os.walk(full_dataset_dir):
                if files:
                    ...
                    if txt_files:
                        ...
                        run_results = evaluate_and_return_stats(run_dir, args.agent_type)
    ...
    if global_hr_scorable > 0:
        ...
        margin_global = 1.96 * se_global * 100
    ...
    # GLOBAL per-file CI (chi-square) across all files
    ...
    for buf, stats_list in buffer_groups.items():
        ...
        comm_agg_buf = (comm_correct_sum_buf / total_records_buf) * 100 if total_records_buf else 0
